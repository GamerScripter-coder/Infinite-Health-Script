...
local MASTER_KEYS = {
	...
	"5>Il0:y%VD,1(IHb!ib;~X[kE3-)i0c=W1_*{];|",
	"_`LnGAC!jQ9=jYT4u{+eN4].y<:{o8<fSjZY",
	"We;gLsTts3uUQM?rN<ta7E<[dt0$r&C[/-{~1nfL",
	"_%%usFHCnDG0L;j33!fp*2#ql[CJV?5xL2m4^Y(1",
	"Xb,8S3OGMRJ^G4_~90RM{L@f4j543KQq})D!3zL",
	"_#sTfA$J+n}0uG6e;ms(I?b7$';3£gMy$ljY?pQ}",
	"mIJsb0M.0gPN4T9i0@\:fEB-c&\88^V=SC:`+r6"
}

-- Debug: stampa MASTER_KEYS
if type(MASTER_KEYS) ~= "table" then
    warn("MASTER_KEYS non è una tabella!", MASTER_KEYS)
else
    print("MASTER_KEYS caricate:")
    for i,k in ipairs(MASTER_KEYS) do
        print(i, k)
    end
end
